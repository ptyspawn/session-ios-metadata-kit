...
public final class LokiSessionCipher : SessionCipher {
    ...
    @objc public init(sessionResetImplementation: SessionResetProtocol, sessionStore: SessionStore, preKeyStore: PreKeyStore, signedPreKeyStore: SignedPreKeyStore, identityKeyStore: IdentityKeyStore, recipientID: String, deviceID: Int32) {
    ...
    override public func decrypt(_ whisperMessage: CipherMessage, protocolContext: Any?) throws -> Data {
        ...
        if (currentState == nil && whisperMessage.cipherMessageType == .prekey) {
            try sessionResetImplementation?.validatePreKeyForFriendRequestAcceptance(for: recipientID, whisperMessage: whisperMessage, protocolContext: protocolContext)
        }
